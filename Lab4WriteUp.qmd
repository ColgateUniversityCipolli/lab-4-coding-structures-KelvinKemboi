...
Printing the odd rows in data frame
```{r}
#| size: "scriptsize"
df[seq(1, 10, 2),]
```
...
creating `calculate.score` function and using for loop to perform operation. Stores the values in `total.score` and return that.
```{r}
#| size: "scriptsize"
...
Using if-statements in the `evaluate.row` function to determine is a number is "high", "low", or "mid"
```{r}
#| size: "scriptsize"
...
**Solution:**
In (h), instead of using the for-loop, I can directly multiply `x1` column and `x2` column and have it in `df$x12.2` through vectorization. `R` will autmoatically compute row by row.
...
In (i), a function and for-loop aren't needed, and I can also directly apply the formula to get the results. I can directly call the columns `df$x1`, `df$x2`, and `df$x3` and pass them into the folrmula and `R` will autmoatically compute row by row
...
In (j), the function is necessary to pass the argument in the conditionals. Since the outcome is dependant on the if-else conditions, I would need the function to get the result of a single value then use `sapply` to apply it to the whole column `df$x1`.  I could also use a for-loop like this:
...
Reading the html website using ``read_html()`
```{r}
#| size: "scriptsize"
...
Pulling a reading the ingredients section `ingredients`
```{r}
#| size: "scriptsize"
...
Finding all itemized items using `html_elements()`
```{r}
#| size: "scriptsize"
...
Pulled the instructions section using `html_element()`
```{r}
#| size: "scriptsize"
...
Pulled all the individual instructions then assigned it to ``instructions.section`
```{r}
#| size: "scriptsize"
...
Pulled the image html by its class using `html_element()`
```{r}
#| size: "scriptsize"
...
Pulled image source link by attribute then pasted "website/" to the link using `paste()`
```{r}
#| size: "scriptsize"
