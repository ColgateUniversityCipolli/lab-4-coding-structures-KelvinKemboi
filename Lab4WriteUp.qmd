...
creating `calculate.score` function and using for loop to perform operation. Stores valu
```{r}
#| size: "scriptsize"
...
**Solution:**
In (h), a for-loop is necessary to fill the rows in the newly created column with product of `x1` and `x2`. In (i), a function is needed to have arguement representations of the `x1`, `x2`, `x3` columns to effeciently calculate the computation. I implemented a for-loop similar to (h) to fill the newly formed `total.score` column with the computed values. In (j), using the `evaluate.row` function...
